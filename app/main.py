...
from app.routers import webhook, onboarding, panel, instagram_webhook, health, tenant_api, billing
...
async def google_oauth_callback(code: str = "", state: str = "", error: str = ""):
    """OAuth 2.0 callback: exchange code for tokens, save to Supabase, notify owner."""
    if error:
        logger.warning(f"[Google OAuth] Erro: {error}")
        return HTMLResponse("<html><body><h2>Autorização cancelada.</h2><p>Pode fechar esta janela.</p></body></html>")
    if not code or not state:
        return HTMLResponse("<html><body><h2>Parâmetros inválidos.</h2></body></html>")
    try:
        from app.services.calendar import exchange_code
        ...
        google_email = info_resp.json().get("email", "") if info_resp.status_code == 200 else ""
        ...
        try:
            owner_row = sb.table("owners").select("phone, business_name").eq("id", owner_id).single().execute()
            if owner_row.data:
                from app.services.whatsapp import WhatsAppService
                wa = WhatsAppService()
                await wa.send_message(
                    owner_row.data["phone"],
                    f"✅ *Google Calendar conectado!*\n\nConta: {google_email}\n\nSeus clientes agora podem agendar reuniões pelo WhatsApp. 🚀"
                )
        except Exception as e:
            logger.error(f"[Google OAuth] Erro ao notificar: {e}")
        
        return HTMLResponse(
            "<html><body style='font-family:sans-serif;text-align:center;padding:40px'>"
            "<h2>✅ Google Calendar conectado!</h2>"
            "<p>Pode fechar esta janela. Confirmação enviada no WhatsApp.</p>"
            "</body></html>"
        )
    except Exception as e:
